...
stories = json.loads(requests.get(API_URL).content)

def story_url(story_id):
    return "https://hacker-news.firebaseio.com/v0/item/{}.json".format(story_id)

def get_top_thirty_stories():
    for story in stories[:30]:
        story_obj = {}
        story = json.loads(requests.get(story_url(story)).content)
        ...
        try:
            for comment in story['kids']:
                comment_obj = {}
                comment = json.loads(requests.get(story_url(comment)).content)
